...
<div class="element-borrow-book hidden">
        ...
        <input type="text" placeholder="Nhập thời hạn mượn(ngày) - mặc định 14..." class="quantity-borrow-day">
...
const booksApi = 'http://localhost:8080/api/v1/book/list'
...
function handleAddBook(listDataBook) {
    ...
    elementSubmitAddBook.onclick = function () {
        if (listID.includes(Number(valueID.value))) {
            ...
            if (!(Number(valueQuantity.value) > 0)) {
                ...
                fetch("http://localhost:8080/api/v1/book/create-book", options)
...
function handleDeleteBook(id, bookId) {
    ...
    // URL và id của request
    const url = `http://localhost:8080/api/v1/book/delete-book/${bookId}`; // Thay thế 123 bằng giá trị id tương ứng
    const method = "DELETE";

    // Mở kết nối với phương thức DELETE
